...
def test_sample_2d_benchmark(benchmark):
    benchmark(set_up_sample_2d_test)
...
class TestSample2D(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        particle_data, split = set_up_sample_2d_test()
    ...
    def test_sample_2d_vector_1_centers(self):
    ...
    def test_sample_2d_vector_1_bounds(self):
        """Test vector 1 bounds in split data."""

        assert all(round(a, 8) == b for a, b 
                   in zip(self.split.vector_1_bounds,
                     [-0.05842331, -0.02930233, -0.00018136, 0.02893961])
                   )
        
        
    def test_sample_2d_vector_2_centers(self):
    ...
    def test_sample_2d_vector_2_bounds(self):
        """Test vector 2 bounds in split data."""

        assert all(round(a, 8) == b for a, b 
                   in zip(self.split.vector_2_bounds,
                      [-0.02860422, 0.00097172, 0.03054767, 0.06012362])
                   )
        

    def test_sample_2d_vector_3_centers(self):
        """Test vector 3 cell centers in split data."""

        assert self.split.vector_3_centers is None


    def test_sample_2d_vector_3_bounds(self):
